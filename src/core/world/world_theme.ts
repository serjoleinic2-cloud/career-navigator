...
export type WorldPalette = {
  ...
  secondary: string;
  accent: string;
  glowRGB: string; // "r, g, b" для использования в rgba()
...
export function getWorldCssVars(theme: WorldTheme, chapterId?: string): Record<string, string> {
  const chapterAccent = chapterId ? getChapterAccent(theme, chapterId) : theme.palette.primary;
  const chapterBg = chapterId ? getChapterBackground(theme, chapterId) : theme.palette.backgroundTo;
