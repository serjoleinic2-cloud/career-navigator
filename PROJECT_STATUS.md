...
### 2026-07-13 — Claude (баг: последнее задание в главе блокировалось при повторном входе через World/Next)

Serj сообщил: кнопка Test (Settings) переносит на Offer, последнюю главу можно пройти. Но если после этого зайти в World, выбрать любой остров и дойти кнопками Next обратно до Offer — последнее задание оказывается заблокировано и непроходимо.

**Причина:** `setActiveChapter()` (`core/runtime/runtime_controller.ts`) — единая функция, которую используют и тап по острову на World-карте, и кнопки "Next"/"Back" в Journey — всегда ставила `activeNodeId` на **первый** узел главы (`chapter.nodeIds[0]`), не глядя на реальный прогресс. Это нормально для первого захода в свежую главу (там первый узел и должен активироваться), но ломается при повторном входе в уже начатую/дев-завершённую главу: `devCompleteAllExceptLastTask()` (кнопка Test) ставит все узлы кроме последнего в `confidence`, а последний — в `awareness` и делает его `activeNodeId`, поэтому сразу после Test всё работает. Но стоило пройти World → любой остров → Next...Next до Offer, как `setActiveChapter('offer')` перезаписывал `activeNodeId` на первый (уже пройденный) узел Offer. Настоящий последний узел переставал быть `activeNodeId`, а его состояние `awareness` не входит в число "выполнено" (`confidence`/`execution`) — `ChapterHub.getNodeCardState()` рисовал его как `locked`, и открыть задание было невозможно.

**Исправлено:** `setActiveChapter()` теперь ищет первый ещё не завершённый узел главы (`state !== 'confidence' && state !== 'execution'`) вместо жёсткого `nodeIds[0]`; если все узлы завершены — берёт последний узел; если прогресса вообще нет — по-прежнему первый (совпадает со старым поведением для свежих глав, регрессии нет).

**Файлы:** `src/core/runtime/runtime_controller.ts`, `PROJECT_STATUS.md`
...
### 2026-07-13 — Claude (проверка бага Settings→Test для всех профессий + устранение дублирования логики + мелкая уборка)

Serj повторно сообщил про баг: Settings → Test → в последней главе последнее задание не активно. Проверил во всех трёх профессиях.

**Проверка:** прогнал полную симуляцию реального кода (не по догадке, а вызвав настоящие `devCompleteAllExceptLastTask`, `saveRuntime`/`initializeRuntime` через JSON-round-trip, и полную навигацию World → каждый остров по очереди → обратно в последнюю главу через `setActiveChapter`) для software_engineer, data_analyst и cybersecurity. Во всех трёх `activeNodeId` после Test и после полного цикла навигации совпадает с последним узлом главы, а его состояние `awareness` (то есть карточка должна рендериться как `current`, не `locked`).

**Важный контекст:** этот же баг ("после Test → World → Next…Next обратно в Offer последнее задание блокируется") уже был найден и исправлен раньше сегодня, в `setActiveChapter()` — она перестала жёстко прыгать на `nodeIds[0]` и теперь ищет первый незавершённый узел главы. Если баг всё ещё воспроизводится на телефоне — вероятно, тестируется сборка до этого фикса, нужно пересобрать APK с актуального `main`.

**Дополнительно исправлено (для профилактики):**
- `devCompleteAllExceptLastTask()` раньше сама, второй независимой реализацией, решала "на каком узле оказаться" (дублируя логику `setActiveChapter()`) — именно расхождение двух независимых реализаций одного и того же и стало причиной прошлого бага. Теперь `devCompleteAllExceptLastTask()` только выставляет состояния узлов/прогресс глав, а решение "куда поставить activeNodeId" полностью делегирует `setActiveChapter()` — теперь на это есть только одна реализация, значит только одно место, где это может сломаться.
- `storage.ts`: `remove()` и `exists()` не были обёрнуты в try/catch (в отличие от `load()`/`save()`), то есть падали необработанным исключением, если localStorage вдруг недоступен. Приведены к тому же паттерну try/catch + fallback на in-memory store.
- `startJourney()` (`runtime_controller.ts`): убраны два забытых отладочных `console.log` ("runtimeState keys", "can stringify"), засорявших консоль при каждом старте нового прохождения.

**Файлы:** `src/core/runtime/runtime_controller.ts`, `src/core/persistence/storage.ts`, `PROJECT_STATUS.md`.

**Проверено:** `npx tsc --noEmit` — 26 ошибок, без изменений (всё в `cybersecurity/skill_nodes_extended.ts`, не связаны). `npx vite build` — чисто. Симуляция реального кода (см. выше) — совпадение для всех 3 профессий и сразу после Test, и после полной навигации по всем главам и обратно.
**Не проверено вживую на телефоне.** Проверить на актуальной сборке: (1) Settings → Test → последнее задание в Offer активно и проходится. (2) То же + предварительно зайти в World, походить по всем островам, вернуться в Offer через Next — задание всё ещё активно. (3) Проверить на всех трёх профессиях отдельно (переключать профессию через новую игру/онбординг).

### 2026-07-13 — Claude (мульти-профессия: раздельный прогресс + переключатель в Profile)

Serj спросил моё мнение: если юзер купил 2+ профессии и хочет проходить каждую частями, у него сейчас нет способа переключиться между ними без потери прогресса. Согласился, что это нужно, и что дело не только в UI — реализовал.

**Была найдена более глубокая проблема:** весь прогресс (`JourneyRuntimeState`) хранился под ОДНИМ глобальным ключом `career-navigator.runtime.v1` — то есть в приложении физически мог существовать прогресс только по одной профессии одновременно. `switchProfession()` при смене профессии не сохранял прогресс старой — он полностью стирал рантайм и создавал пустой под новую профессию (`createEmptyRuntime()`).

**Исправлено — хранение стало по-профессионально:**
- `runtime_persistence.ts` переписан: вместо одного ключа теперь `career-navigator.runtime.<professionId>.v1` — свой ключ на каждую профессию, плюс отдельный маленький ключ-указатель `career-navigator.activeProfessionId.v1` ("какая профессия сейчас активна", чтобы обычный холодный старт продолжал "просто резюмиться" без необходимости знать, какую профессию спрашивать).
- Добавлена **автоматическая миграция**: если у пользователя есть старый прогресс под старым единым ключом — при первой загрузке после обновления он переносится в новую по-профессиональную схему один раз, старый ключ удаляется. Проверено симуляцией — прогресс не теряется.
- `switchProfession()` (`runtime_controller.ts`) переписан: больше не стирает прогресс. Если у целевой профессии уже есть сохранённый прогресс — загружает его (`initializeRuntime`); если профессия открывается впервые — стартует с нуля (как обычный онбординг), не трогая прогресс остальных профессий.
- `clearAllPersistence()` (ранее нигде не вызывалась, но была неверна) обновлена: теперь чистит ключи всех зарегистрированных профессий + указатель активной, а не только старый единый ключ.

**Добавлен переключатель профессий в Profile:**
- Новый хелпер `src/core/premium/profession_progress_summary.ts` → `getOwnedProfessionsSummary()`: список купленных профессий (`getOwnedProfessionIds()` из `entitlements.ts` — пока заглушка "все зарегистрированные", реальный биллинг подключится позже без переделки этого кода) с процентом прохождения каждой (0%, если не начата) и отметкой текущей активной.
- `ProfileScreen.tsx`: добавлена секция "My Professions" — по карточке на каждую купленную профессию с прогресс-баром, тап переключает (`switchProfession`). Показывается только если куплено больше одной профессии (иначе это просто дублирует то, что уже видно в паспорте профиля).

**Проверено полной симуляцией реального кода:** прогресс в Software Engineer (после старта) сохраняется независимо; переключение в Data Analyst (впервые) стартует его с нуля; переключение ОБРАТНО в Software Engineer восстанавливает именно тот прогресс, что был; Cybersecurity, ни разу не открытая, корректно показывает "не начата" с 0%. Миграция со старого единого ключа — прогресс восстанавливается и переносится в новую схему.

**Файлы:** `src/core/persistence/runtime_persistence.ts`, `src/core/runtime/runtime_controller.ts`, `src/core/premium/profession_progress_summary.ts` (создан), `src/screens/ProfileScreen/ProfileScreen.tsx`, `src/screens/ProfileScreen/ProfileScreen.css`, `PROJECT_STATUS.md`.

**Проверено:** `npx tsc --noEmit` — 26 ошибок, без изменений (всё в `cybersecurity/skill_nodes_extended.ts`, не связаны). `npx vite build` — чисто.
**Не проверено вживую на телефоне.** Проверить: (1) Пройти немного в Software Engineer → Profile → переключиться на Data Analyst → прогресс Data Analyst стартует с нуля. (2) Profile → переключиться обратно на Software Engineer → старый прогресс на месте, не сброшен. (3) Существующий (до этого изменения) прогресс на реальном телефоне не потерялся после обновления (миграция сработала). (4) Профессия, которую никогда не открывали, показывает "Not started"/0% в списке, а не отсутствует из списка.
